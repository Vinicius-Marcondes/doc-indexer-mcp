...
| Remote HTTP remains docs-only | `src/http.ts`, `src/http/mcp.ts`, `src/server.ts` | `tests/unit/http-import-boundary.test.ts`, `tests/integration/mcp/streamable-http-entrypoint.test.ts`, `tests/integration/mcp/server-registration.test.ts` | done |
| Admin console remains a separate optional process | `apps/admin-console/server`, `apps/admin-console/client`, `docker-compose.yml` | `tests/unit/deployment/docker-config.test.ts`, `tests/unit/admin` | done |
