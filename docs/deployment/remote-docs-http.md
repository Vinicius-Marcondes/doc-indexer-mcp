...
This repository is intentionally HTTP/docs-only. Local stdio project analysis lives in the split-out `bun-dev-intel-stdio-mcp` repository, and the admin console lives in the split-out `bun-dev-intel-admin-console` repository.
...
On-demand refresh can be queued by docs tools when content is missing, stale, or low confidence. These jobs are bounded, deduplicated by source/URL/job type, and processed by `bun src/docs-worker.ts`.
